...
if (@searchpatron) {
    ($results) = Search( \@searchpatron, [ { surname => 0 }, { firstname => 0 } ], undef, undef, [ "firstname", "surname", "email", "othernames", "cardnumber" ], "start_with" );
}
if ($results) {
...
my $base_url = 'member-search.pl?&amp;' . join( '&amp;', map { "$_=$parameters{$_}" } ( keys %parameters ) );
