...
sub no_parcels : Test( 1 ) {
    ...
    my @parcels = GetParcels(
        $self->{'booksellerid'},    # bookseller
                                    # order
...
sub one_parcel : Test( 19 ) {
    ...
    my $today = sprintf( '%04d-%02d-%02d', localtime->year() + 1900, localtime->mon() + 1, localtime->mday() );

    $self->create_order(
        authorizedby => 1,                                                                                        # XXX what should this be?
        invoice      => $invoice,
        date         => $today
    );
...
sub z_several_parcels_with_different_dates : Test( 44 ) {
    ...
    my @inputs = (
        {   invoice => 10,
            date    => sprintf( '%04d-%02d-%02d', 1950, localtime->mon() + 1, 10 ),    # I'm using the invoice number as the day.
        },
        {   invoice => 15,
            date    => sprintf( '%04d-%02d-%02d', 1950, localtime->mon() + 1, 15 ),    # I'm using the invoice number as the day.
        },
        {   invoice => 20,
            date    => sprintf( '%04d-%02d-%02d', 1950, localtime->mon() + 1, 20 ),    # I'm using the invoice number as the day.
        },
    );
    ...
    my @parcels = GetParcels(
        ...
        undef,                                                                         # order
        undef,                                                                         # code ( aqorders.booksellerinvoicenumber )
        sprintf( '%04d-%02d-%02d', 1950, localtime->mon() + 1, 10 ),                   # datefrom
        sprintf( '%04d-%02d-%02d', 1950, localtime->mon() + 1, 20 ),                   # dateto
    );
    ...
    @parcels = GetParcels(
        ...
        undef,                                                                         # code ( aqorders.booksellerinvoicenumber )
        sprintf( '%04d-%02d-%02d', 1950, localtime->mon() + 1, 10 ),                   # datefrom
        sprintf( '%04d-%02d-%02d', 1950, localtime->mon() + 1, 16 ),                   # dateto
    );
...
sub create_order {
    ...
    $param{'date'} = sprintf( '%04d-%02d-%02d', localtime->year() + 1900, localtime->mon() + 1, localtime->mday() ) unless exists $param{'date'};

    my ( $basketno, $ordernumber ) = $self->create_new_basket(%param);

    my $datereceived = ModReceiveOrder(
        $self->{'biblios'}[0],    # biblionumber
