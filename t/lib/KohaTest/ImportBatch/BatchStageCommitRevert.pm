...
sub startup_60_make_test_records : Test( startup ) {
    ...
    $self->{'batches'} = {
        ...
        'batch2' => {
            marc => _make_marc_batch(
                [ [ 'isbn001', 'overlay title 1', ['batch-item-8'] ], [ 'isbn002', 'overlay title 2', ['batch-item-9'] ], [ 'isbn006', 'title 6', ['batch-item-10'] ], ]
            ),
            args => {
                parse_items    => 1,
        ...
        'batch3' => {
            marc => _make_marc_batch( [ [ 'isbn007', 'title 7', ['batch-item-11'] ], [ 'isbn006', 'overlay title 6', ['batch-item-12'] ], ] ),
            args => {
                parse_items    => 1,
        ...
        'batch6' => {
            marc =>
              _make_marc_batch( [ [ 'isbn001', 'match title 1', [ 'batch-item-14', 'batch-item-15' ] ], [ 'isbn010', 'title 10', [ 'batch-item-16', 'batch-item-17' ] ], ] ),
            args => {
                parse_items    => 1,
                overlay_action => 'ignore',
                nomatch_action => 'create_new',
...
sub _make_marc_batch {
    ...
    foreach my $rec (@$defs) {
        if ( ref($rec) eq 'ARRAY' ) {
            ...
            $bib->append_fields( MARC::Field->new( '020', ' ', ' ', a => $isbn ), MARC::Field->new( '245', ' ', ' ', a => $title ) );
...
sub stage_commit_batches : Test( 75 ) {
    ...
    for my $batch_key ( sort keys %{ $self->{'batches'} } ) {
        ...
        my ( $batch_id, $num_bibs, $num_items, @invalid ) =
          BatchStageMarcRecords( 'MARC21', $batch->{marc}, "$batch_key.mrc", "$batch_key comments", '', $args->{'parse_items'}, 0 );
        ...
        if ( defined $args->{'overlay_action'} ) {
            if ( $args->{'overlay_action'} eq 'create_new' ) {
                ...
                cmp_ok( GetImportBatchOverlayAction($batch_id), "eq", $args->{'overlay_action'}, "$batch_key: changed overlay action" );
            }
        }
        if ( defined $args->{'nomatch_action'} ) {
            if ( $args->{'nomatch_action'} eq 'create_new' ) {
                ...
                cmp_ok( GetImportBatchNoMatchAction($batch_id), "eq", $args->{'nomatch_action'}, "$batch_key: changed nomatch action" );
            }
        }
        if ( defined $args->{'item_action'} ) {
            if ( $args->{'item_action'} eq 'create_new' ) {
                ...
                cmp_ok( GetImportBatchItemAction($batch_id), "eq", $args->{'item_action'}, "$batch_key: changed item action" );
            }
        }

        my ( $num_added, $num_updated, $num_items_added, $num_items_errored, $num_ignored ) = BatchCommitBibRecords($batch_id);
