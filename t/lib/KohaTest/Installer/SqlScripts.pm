...
sub installer_all_sample_data : Tests {
    ...
    my $dir = C4::Context->config('intranetdir') . "/installer/data/" . ( C4::Context->config("db_scheme") ? C4::Context->config("db_scheme") : "mysql" ) . "/";
    opendir( MYDIR, $dir );
            ...
            foreach my $flavour (@$marc_flavours) {
              SKIP: {
                    ...
                    foreach my $sql (@file_list) {
                        ok( exists( $sql_to_load{"$sql->{level}\t$sql->{fwkname}"} ), "SQL script $sql->{level}/$sql->{fwkname} is on list" );
