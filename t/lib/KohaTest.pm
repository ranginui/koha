...
sub startup_30_login : Test( startup => 2 ) {
    ...
    my $cgi = CGI->new(
        {   userid   => $borrower_details->{'cardnumber'},
            password => 'testpassword'
        }
    );
    my $setcookie = $cgi->cookie(
        -name  => 'CGISESSID',
...
sub days_from_now {
    ...
    my $yyyymmdd = sprintf( '%04d-%02d-%02d', localtime($seconds)->year() + 1900, localtime($seconds)->mon() + 1, localtime($seconds)->mday() );
    return $yyyymmdd;
}
...
sub add_biblios {
    ...
    foreach my $counter ( 1 .. $param{'count'} ) {
        ...
        my @marc_fields = (
            ...
            MARC::Field->new( '952', '0', '0', p => '12345678' . $self->random_string() ),    # barcode
            MARC::Field->new( '952', '0', '0', o => $self->random_string() ),                 # callnumber
            MARC::Field->new(
                '952', '0', '0',
                a => 'CPL',
                b => 'CPL'
...
sub create_test_database {
    ...
    my $list = $installer->sql_file_list(
        'en', 'marc21',
        {   optional  => 1,
            mandatory => 1
        }
...
sub start_zebrasrv {
    ...
    my $command = sprintf( 'zebrasrv -f %s -D -l %s -p %s', $ENV{'KOHA_CONF'}, File::Spec->catdir( C4::Context->config("logdir"), 'zebra.log' ), $pidfile, );
    diag $command;
