...
sub plugin {
    ...
    if ( $op eq "do_search" ) {
        ...
        ( $template, $loggedinuser, $cookie ) = get_template_and_user(
            {   template_name   => "value_builder/labs_theses.tmpl",
                query           => $query,
                type            => 'intranet',
                authnotrequired => 1,
                debug           => 1,
        ...
        $template->param( 'search' => $query->param('search'), );
        $template->param( 'index' => '' . $query->param('index') ) if ( $query->param('index') );
        warn 'index : ' . $query->param('index');
        $template->param( 'cat_auth' => $cat_auth ) if ($cat_auth);
    }
