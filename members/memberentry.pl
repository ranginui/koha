...
my ( $template, $loggedinuser, $cookie ) = get_template_and_user(
    {   template_name   => "members/memberentrygen.tmpl",
        query           => $input,
        type            => "intranet",
        authnotrequired => 0,
...
{
    my @keys_to_delete = (
        qr/^BorrowerMandatoryField$/, qr/^category_type$/, qr/^check_member$/,                       qr/^destination$/,
        qr/^nodouble$/,               qr/^op$/,            qr/^save$/,                               qr/^select_roadtype$/,
        qr/^updtype$/,                qr/^SMSnumber$/,     qr/^setting_extended_patron_attributes$/, qr/^setting_messaging_prefs$/,
        qr/^digest$/,                 qr/^modify$/,        qr/^step$/,                               qr/^\d+$/,
        qr/^\d+-DAYS/,                qr/^patron_attr_/,
    );
...
if ( ( !$nok ) and $nodouble and ( $op eq 'insert' or $op eq 'save' ) ) {
    ...
    if ( $op eq 'insert' ) {
        ...
        &ModMember(%newdata) if ( %newdata && scalar( keys %newdata ) );
        if ( C4::Context->preference('ExtendedPatronAttributes') and $input->param('setting_extended_patron_attributes') ) {
            C4::Members::Attributes::SetBorrowerAttributes( $borrowernumber, $extended_patron_attributes );
        }
        if ( C4::Context->preference('EnhancedMessagingPreferences') and $input->param('setting_messaging_prefs') ) {
        ...
        push @categoryloop,
          { 'categorycode' => $cat,
            ...
            'categorycodeselected' =>
              ( ( defined( $borrower_data->{'categorycode'} ) && $cat eq $borrower_data->{'categorycode'} ) || ( defined($categorycode) && $cat eq $categorycode ) ),
          };
    }
    my %typehash;
    $typehash{'typename'}     = $_;
...
my $borrotitlepopup = CGI::popup_menu(
    -name     => 'title',
    -id       => 'btitle',
...
my $onlymine =
  ( C4::Context->preference('IndependantBranches') && C4::Context->userenv && C4::Context->userenv->{flags} % 2 != 1 && C4::Context->userenv->{branch} ? 1 : 0 );
...
if ( C4::Context->preference("memberofinstitution") ) {
    ...
    $CGIorganisations = CGI::scrolling_list(
        -id       => 'organisations',
        -name     => 'organisations',
        -labels   => \%org_labels,
        -values   => \@orgs,
