...
sub create_input {
    ...
    ";
        ...
        my $extended_param;
        eval { $extended_param = plugin_parameters( $dbh, $rec, $tagslib, $subfield_data{id}, $tabloop ); };
        ...
        if (length($value) > 100
            ...
            or (    $tag >= 600
                and $tag < 700 && C4::Context->preference("marcflavour") eq "MARC21" )
          ) {
            ...
            if ( $record != -1 && ( $record->field($tag) || $tag eq '000' ) ) {
                ...
                foreach my $subfield ( sort( keys %{ $tagslib->{$tag} } ) ) {
                    ...
                    next if ( ( $tagslib->{$tag}->{$subfield}->{hidden} <= -5 )
                        or ( $tagslib->{$tag}->{$subfield}->{hidden} >= 4 ) );    #check for visibility flag
                    next if ( $tagslib->{$tag}->{$subfield}->{tab} ne $tabloop );
                    push( @subfields_data, &create_input( $tag, $subfield, '', $index_tag, $tabloop, $record, $authorised_values_sth, $input ) );
                }
                if ( $#subfields_data >= 0 ) {
...
my ( $template, $loggedinuser, $cookie ) = get_template_and_user(
    {   template_name   => "authorities/authorities.tmpl",
        query           => $input,
        type            => "intranet",
        authnotrequired => 0,
