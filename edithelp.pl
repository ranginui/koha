...
if ( $type eq 'addnew' ) {
    ...
    unless ( open( OUTFILE, ">$file" ) ) { $error = "Cannot write file: '$file'"; }
    else {
    ...
    if ( !-r $file ) {
        ...
        ( -w $file ) or $error = "WARNING: You will not be able save, because your webserver cannot write to '$file'. Contact your admin about help file permissions.";
