...
sub _marc_marc8_to_utf8 {
    ...
    foreach my $field ( $marc_record->fields() ) {
        if ( $field->is_control_field() ) {
            ...
            $field->replace_with( MARC::Field->new( $field->tag(), $field->indicator(1), $field->indicator(2), @converted_subfields ) );
        }
    }
...
=head2 nsb_rm_content

=over 4

nsb_rm_content($string);

=back

Removes Non Sorting Block characters and its content
