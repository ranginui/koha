        ...
        if ( ( $string_width + $text_attribs->{'llx'} ) > $self->{'width'} ) {
          WRAP_LINES:
            while (1) {
                ...
                $line =~ m/^.*(\s.*\s*|\s&|\<.*\>)$/;
                warn sprintf(
'Line wrap failed. DEBUG INFO: Data: \'%s\'\n Method: C4::Patroncards->draw_text Additional Information: Line wrap regexp failed. (Please file in this information in a bug report at http://bugs.koha.org',
                    $line )
                  and last WRAP_LINES
                  if !$1;
                $trim = $1 . $trim;
...
sub draw_image {
  ...
  PROCESS_IMAGES:
    foreach my $image ( keys %$images ) {
        ...
        my $intName = $pdf->AltJpeg(
            $images->{$image}->{'data'},
            $images->{$image}->{'Sx'},
            $images->{$image}->{'Sy'},
            1,
            $images->{$image}->{'alt'}->{'data'},
            $images->{$image}->{'alt'}->{'Sx'},
            $images->{$image}->{'alt'}->{'Sy'}, 1
        );
        my $obj_stream = "q\n";
