...
sub _check_params {
    my $given_params          = {};
    my $exit_code             = 0;
    my @valid_template_params = ( 'label_id', 'batch_id', 'item_number', 'card_number', 'branch_code', 'creator', );
    if ( scalar(@_) > 1 ) {
        $given_params = {@_};
...
sub remove_duplicates {
    ...
    foreach my $item (@duplicate_items) {
        ...
        @{ $self->{'items'} } =
          ...
          @{ $self->{
                'items'} };   # the correct label/item must be removed from the current batch object as well; this should be done *after* each sql DELETE in case the DELETE fails
    }
