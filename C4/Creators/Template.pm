...
sub _check_params {
    ...
    my @valid_template_params = (
        'profile_id',  'template_code',   'template_desc',    'page_width', 'page_height', 'label_width', 'label_height', 'card_width',
        'card_height', 'top_text_margin', 'left_text_margin', 'top_margin', 'left_margin', 'cols',        'rows',         'col_gap',
        'row_gap',     'units',           'creator',          'current_label',
    );
    if ( scalar(@_) > 1 ) {
        $given_params = {@_};
...
sub get_label_position {
    ...
    if ( $start_label eq 1 ) {
        ...
        $current_label->{'llx'} =
          $self->{'left_margin'} + ( $self->{'label_width'} * ( $current_label->{'col_count'} - 1 ) ) + ( $self->{'col_gap'} * ( $current_label->{'col_count'} - 1 ) );
        $current_label->{'lly'} =
          $self->{'page_height'} -
          $self->{'top_margin'} -
