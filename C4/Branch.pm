...
use vars qw($VERSION @ISA @EXPORT @EXPORT_OK %EXPORT_TAGS $cache $usecache);
...
sub GetBranches {
    ...
    my %branches;
    if ($usecache){
	my $cachedbranch = $cache->get_from_cache('koha:branches');
	if ($cachedbranch){
	    return $cachedbranch;	    
	}
    }
    ...
    if ($usecache){
	$cache->set_in_cache('koha:branches',\%branches,300);
    }
    return ( \%branches );
}
