...
sub AddImportBatch {
    ...
    my $dbh = C4::Context->dbh;
    my $sth = $dbh->prepare(
        "INSERT INTO import_batches (overlay_action, import_status, batch_type,
                                                         file_name, comments)
                                    VALUES (?, ?, ?, ?, ?)"
    );
...
sub BatchStageMarcRecords {
    ...
    foreach my $marc_blob ( split( /\x1D/, $marc_records ) ) {
        ...
        my ( $marc_record, $charset_guessed, $char_errors ) = MarcToUTF8Record( $marc_blob, C4::Context->preference("marcflavour") );
        my $import_record_id;
...
sub AddItemsToImportBiblio {
    ...
    foreach my $item_field ( $marc_record->field($item_tag) ) {
        ...
        my $sth = $dbh->prepare_cached(
            "INSERT INTO import_items (import_record_id, status, marcxml)
                                        VALUES (?, ?, ?)"
        );
...
sub BatchCommitBibRecords {
    ...
    my $dbh            = C4::Context->dbh;
    my $sth            = $dbh->prepare(
        "SELECT import_record_id, status, overlay_status, marc, encoding
                             FROM import_records
                             JOIN import_biblios USING (import_record_id)
                             WHERE import_batch_id = ?"
    );
    ...
    while ( my $rowref = $sth->fetchrow_hashref ) {
        ...
        my ( $bib_result, $item_result, $bib_match ) =
          _get_commit_action( $overlay_action, $nomatch_action, $item_action, $rowref->{'overlay_status'}, $rowref->{'import_record_id'} );
...
sub BatchRevertBibRecords {
    ...
    my $dbh            = C4::Context->dbh;
    my $sth            = $dbh->prepare(
        "SELECT import_record_id, status, overlay_status, marcxml_old, encoding, matched_biblionumber
                             FROM import_records
                             JOIN import_biblios USING (import_record_id)
                             WHERE import_batch_id = ?"
    );
...
sub BatchRevertItems {
    ...
    my $sth = $dbh->prepare_cached(
        "SELECT import_items_id, itemnumber
                                   FROM import_items
...
sub GetItemNumbersFromImportBatch {
    ...
    my $dbh        = C4::Context->dbh;
    my $sth        = $dbh->prepare(
"SELECT itemnumber FROM import_batches,import_records,import_items WHERE import_batches.import_batch_id=import_records.import_batch_id AND import_records.import_record_id=import_items.import_record_id AND import_batches.import_batch_id=?"
    );
    $sth->execute($batch_id);
    my @items;
...
sub GetImportRecordMatches {
    ...
    my $sth = $dbh->prepare_cached(
        "SELECT title, author, biblionumber, score
                                    FROM import_records
...
sub SetImportRecordMatches {
    ...
    my $sth = $dbh->prepare(
        "INSERT INTO import_record_matches (import_record_id, candidate_match_id, score)
                                    VALUES (?, ?, ?)"
    );
...
sub _create_import_record {
    ...
    my $dbh = C4::Context->dbh;
    my $sth = $dbh->prepare(
        "INSERT INTO import_records (import_batch_id, record_sequence, marc, marcxml, 
                                                         record_type, encoding, z3950random)
                                    VALUES (?, ?, ?, ?, ?, ?, ?)"
    );
    $sth->execute( $batch_id, $record_sequence, $marc_record->as_usmarc(), $marc_record->as_xml(), $record_type, $encoding, $z3950random );
...
sub _update_import_record_marc {
    ...
    my $dbh = C4::Context->dbh;
    my $sth = $dbh->prepare(
        "UPDATE import_records SET marc = ?, marcxml = ?
                             WHERE  import_record_id = ?"
    );
...
sub _update_biblio_fields {
    ...
    $isbn = uc $isbn;
    my $sth = $dbh->prepare(
        "UPDATE import_biblios SET title = ?, author = ?, isbn = ?, issn = ?
                             WHERE  import_record_id = ?"
    );
...
sub _update_batch_record_counts {
    ...
    my $dbh = C4::Context->dbh;
    my $sth = $dbh->prepare_cached(
        "UPDATE import_batches SET num_biblios = (
                                    SELECT COUNT(*)
                                    FROM import_records
    ...
    $sth = $dbh->prepare_cached(
        "UPDATE import_batches SET num_items = (
                                    SELECT COUNT(*)
                                    FROM import_records
