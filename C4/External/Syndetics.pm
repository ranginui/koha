...
sub get_syndetics_index {
    ...
    $response = $xmlsimple->XMLin( $content, ) unless !$content;

    my $syndetics_elements;
    for my $available_type (
        'SUMMARY',  'TOC',      'FICTION',   'AWARDS1',  'SERIES1',  'SPSUMMARY', 'SPREVIEW', 'AVPROFILE',      'AVSUMMARY', 'DBCHAPTER',
        'LJREVIEW', 'PWREVIEW', 'SLJREVIEW', 'CHREVIEW', 'BLREVIEW', 'HBREVIEW',  'KIREVIEW', 'CRITICASREVIEW', 'ANOTES'
      ) {
...
sub get_syndetics_toc {
    ...
    my $content   = $response->content;
    my $xmlsimple = XML::Simple->new();
    $response = $xmlsimple->XMLin( $content, forcearray => [qw(Fld970)], ) unless !$content;
...
sub get_syndetics_excerpt {
    ...
    my $content   = $response->content;
    my $xmlsimple = XML::Simple->new();
    $response = $xmlsimple->XMLin( $content, forcearray => [qw(Fld520)], ) unless !$content;
...
sub get_syndetics_editions {
    ...
    my $xmlsimple = XML::Simple->new();
    $response = $xmlsimple->XMLin( $content, forcearray => [qw(Fld020)], ) unless !$content;
