...
sub handle_hold {
    ...
    if ( $status->ok ) {
        ...
        ( $status->expiration_date )
          and $resp .= maybe_add( FID_EXPIRATION, Sip::timestamp( $status->expiration_date ) );
...
sub handle_renew {
    ...
    if ( $no_block eq 'Y' ) {
        syslog( "LOG_WARNING", "handle_renew: recieved 'no block' renewal from terminal '%s'", $server->{account}->{id} );
    }
