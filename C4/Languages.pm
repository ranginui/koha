...
sub getTranslatedLanguages {
    ...
    @enabled_languages = grep {
        my $enabled_language = $_;
        any { $_ eq $enabled_language } @languages
    } @enabled_languages;
...
sub getAllLanguages {
    ...
    while ( my $language_subtag_registry = $sth->fetchrow_hashref ) {
        ...
        my $sth2 = $dbh->prepare(
"SELECT * FROM language_descriptions LEFT JOIN language_rfc4646_to_iso639 on language_rfc4646_to_iso639.rfc4646_subtag = language_descriptions.subtag WHERE type='language' AND subtag =? AND language_descriptions.lang = ?"
        );
