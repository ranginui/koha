...
sub SimpleSearch {
    ...
    if ( C4::Context->preference('NoZebra') ) {
        ...
        my $search_result =
          ( $result->{hits} && $result->{hits} > 0 ? $result->{'RECORDS'} : [] );
...
sub getRecords {
    my ( $koha_query, $simple_query, $sort_by_ref, $servers_ref, $results_per_page, $offset, $expanded_facet, $branches, $query_type, $scan ) = @_;
...
sub _build_weighted_query {
    ...
    if ( ( $index eq 'kw' ) || ( !$index ) ) {
        $weighted_query .= "Title-cover,first-in-field,phr,r1=\"$operand\"";    # exact title-cover
...
sub searchResults {
    ...
    my %itemtypes;
    $bsth = $dbh->prepare( "SELECT itemtype,description,imageurl,summary,notforloan FROM itemtypes" );
    $bsth->execute();
    while ( my $bdata = $bsth->fetchrow_hashref ) {
    ...
    for ( my $i = $offset ; $i <= $times - 1 ; $i++ ) {
        ...
        $oldbiblio->{content_identifier_exists} = 1
          if ( $oldbiblio->{normalized_isbn} or $oldbiblio->{normalized_oclc} or $oldbiblio->{normalized_ean} or $oldbiblio->{normalized_upc} );
...
sub NZanalyse {
    ...
    if ( $string =~ /^\s*\((.*)\)(( and | or | not | AND | OR | NOT )(.*))?/ ) {
        ...
        warn "dealing w/parenthesis before recursive sub call. left :$left operator:$operator right:$right"
          if $DEBUG;
    ...
    if ($operator) {
        ...
        unless ($operator) {
            if ( $string =~ /(.*)(>|<|=)(.*)/ ) {
                ...
                $operator = $2;
                $right    = $3;
                warn "handling unless (operator)... left:$left operator:$operator right:$right"
                  if $DEBUG;
            } else {
                $left = $string;
            }
        ...
        if ( $operator && $left ne 'keyword' ) {
            ...
            foreach ( split / /, $right ) {
                ...
                while ( my ( $line, $value ) = $sth->fetchrow ) {
                    ...
                    warn "result : $value " . ( $right =~ /\d/ ) . "==" . ( $value =~ /\D/ ? $line : "" ) if $DEBUG;    #= $line";
                }
...
sub GetDistinctValues {
    ...
    if ( $fieldname =~ /\./ ) {
        ...
        my $sth = $dbh->prepare(
            "select DISTINCT($column) as value, count(*) as cnt from $table " . ( $string ? " where $column like \"$string%\"" : "" ) . "group by value order by $column " );
        $sth->execute;
