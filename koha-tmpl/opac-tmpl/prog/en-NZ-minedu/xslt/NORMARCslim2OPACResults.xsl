...
<xsl:if test="$DisplayOPACiconsXSLT!='0'">
 ...
 <span class="label">; Periodical type: :</span>
 </xsl:if>
 <xsl:choose>
 ...
 <span class="label">; Innreserve:</span>
 </xsl:if>
 <xsl:choose>
...
<span class="results_summary">
 ...
 <xsl:choose>
 <xsl:when test="count(key('item-by-status', 'available'))=0 and count(key('item-by-status', 'reference'))=0">No copies available </xsl:when>
