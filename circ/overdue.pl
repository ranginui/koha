...
my $sth = $dbh->prepare(
    'SELECT code,description,repeatable,authorised_value_category
...
if (@patron_attr_filter_loop) {
    ...
    while ( my $row = $sth->fetchrow_hashref ) {
        ...
        push @{ $pattrs->{ $row->{code} } }, [ $row->{val}, defined $row->{avdescription} ? $row->{avdescription} : $row->{val}, ];
    }
    ...
    for my $bn ( keys %borrowernumber_to_attributes ) {
        ...
        if ($debug) {
            ...
            for ( sort keys %$pattrs ) {
                my @a = map { "$_->[0]/$_->[1]  " } @{ $pattrs->{$_} };
                print STDERR "attrcode $_ = [@a] ";
            }
            print STDERR "\n";
        }
...
if ($noreport) {
    ...
    if ( my @attrtype = grep { $_->{'code'} eq $patrorder } @patron_attr_filter_loop ) {    # sort by patron attrs perhaps?
        ...
        sub patronattr_sorter_asc {
            lc $a->{patron_attr_value_loop}[$ordinal]{value} cmp lc $b->{patron_attr_value_loop}[$ordinal]{value};
        }
