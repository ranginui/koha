...
if ($cardnumber) {
    ...
    if (    $expiry_date
        ...
        and Date_to_Days( split( /-/, $date ) ) > Date_to_Days( split( /-/, $expiry_date ) ) ) {
        $messages = $expiry = 1;
    }
...
if ($borrowerslist) {
    ...
    foreach my $borrower ( sort { $a->{surname} . $a->{firstname} cmp $b->{surname} . $b->{firstname} } @{$borrowerslist} ) {
        push @values, $borrower->{cardnumber};

        $labels{ $borrower->{cardnumber} } =
          sprintf( '%s, %s ... (%s - %s) ... %s', $borrower->{surname}, $borrower->{firstname}, $borrower->{cardnumber}, $borrower->{categorycode}, $borrower->{address}, );
    }
...
foreach my $biblionumber (@biblionumbers) {
    ...
    foreach my $biblioitemnumber (@biblioitemnumbers) {
        ...
        $biblioitem->{description}   = $itemtypes->{ $biblioitem->{itemtype} }{description};
        $biblioloopiter{description} = $biblioitem->{description};
        $biblioloopiter{itypename}   = $biblioitem->{description};
        $biblioloopiter{imageurl} = getitemtypeimagelocation( 'intranet', $itemtypes->{ $biblioitem->{itemtype} }{imageurl} );
        ...
        foreach my $itemnumber ( @{ $itemnumbers_of_biblioitem{$biblioitemnumber} } ) {
            ...
            if ( $item->{homebranch} ne $item->{holdingbranch} ) {
                $item->{holdingbranchname} = $branches->{ $item->{holdingbranch} }{branchname};
            }
            ...
            if ( $item->{notforloan} ) {
                ...
                $item->{notforloanvalue} = $notforloan_label_of->{ $item->{notforloan} };
            }
            ...
            my ( $transfertwhen, $transfertfrom, $transfertto ) = GetTransfers($itemnumber);
    ...
    foreach my $res (
        ...
        $reserve{'wait'}            = ( ( defined $res->{'found'} and $res->{'found'} eq 'W' ) or ( $res->{'priority'} eq '0' ) );
