...
my ( $template, $loggedinuser, $cookie ) = get_template_and_user(
    {   template_name   => "admin/auth_tag_structure.tmpl",
        query           => $input,
        type            => "intranet",
        authnotrequired => 0,
...
################## ADD_VALIDATE ##################################
    ...
    unless ( C4::Context->config('demo') eq 1 ) {
        if ( $input->param('modif') ) {
            $sth = $dbh->prepare(
                "UPDATE auth_tag_structure SET tagfield=?, liblibrarian=?, libopac=?, repeatable=?, mandatory=?, authorised_value=? WHERE authtypecode=? AND tagfield=?");
            $sth->execute( $tagfield, $liblibrarian, $libopac, $repeatable, $mandatory, $authorised_value, $authtypecode, $tagfield, );
        } else {
    ...
    print "Content-Type: text/html\n\n<META HTTP-EQUIV=Refresh CONTENT=\"0; URL=auth_tag_structure.pl?searchfield="
...
"select count(*),auth_tag_structure.authtypecode,authtypetext from auth_tag_structure,auth_types where auth_types.authtypecode=auth_tag_structure.authtypecode group by auth_tag_structure.authtypecode"
    ...
    $template->param(
        existingauthtypeloop => \@existingauthtypeloop,
        authtypecode         => $authtypecode,
    );
################## DEFAULT ##################################
} else {    # DEFAULT
...
sub StringSearch {
    ...
    my $sth = $dbh->prepare(
        "Select tagfield,liblibrarian,libopac,repeatable,mandatory,authorised_value from auth_tag_structure where (tagfield >= ? and authtypecode=?) order by tagfield");
...
sub duplicate_auth_framework {
    ...
    my $sth_insert =
      $dbh->prepare("insert into auth_tag_structure  (tagfield, liblibrarian, libopac, repeatable, mandatory, authorised_value, authtypecode) values (?,?,?,?,?,?,?)");
    ...
    $sth = $dbh->prepare(
"select tagfield,tagsubfield,liblibrarian,libopac,repeatable,mandatory,kohafield,tab,authorised_value,value_builder,seealso,hidden from auth_subfield_structure where authtypecode=?"
    );
    $sth->execute($oldauthtype);
    $sth_insert = $dbh->prepare(
"insert into auth_subfield_structure (authtypecode,tagfield,tagsubfield,liblibrarian,libopac,repeatable,mandatory,kohafield,tab,authorised_value,value_builder,seealso,hidden) values (?,?,?,?,?,?,?,?,?,?,?,?,?)"
    );
    while ( my ( $tagfield, $tagsubfield, $liblibrarian, $libopac, $repeatable, $mandatory, $kohafield, $tab, $authorised_value, $thesaurus_category, $seealso, $hidden ) =
        $sth->fetchrow ) {
        $sth_insert->execute(
            $newauthtype, $tagfield, $tagsubfield,      $liblibrarian,       $libopac, $repeatable, $mandatory,
            $kohafield,   $tab,      $authorised_value, $thesaurus_category, $seealso, $hidden
        );
    }
}
