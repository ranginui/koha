...
my ( $template, $loggedinuser, $cookie ) = get_template_and_user(
    {   template_name   => "admin/classsources.tmpl",
        query           => $input,
        type            => "intranet",
        authnotrequired => 0,
...
if ( $op eq "add_source" ) {
    ...
    add_class_source( $template, $source_code, $input->param('description'), $input->param('used') eq "used" ? 1 : 0, $rule_code );
    $display_lists = 1;
    ...
    edit_class_source( $template, $source_code, $input->param('description'), $input->param('used') eq "used" ? 1 : 0, $rule_code );
    $display_lists = 1;
} elsif ( $op eq "add_sort_rule" ) {
    add_class_sort_rule_form($template);
} elsif ( $op eq "add_sort_rule_confirmed" ) {
    add_class_sort_rule( $template, $rule_code, $input->param('description'), $input->param('sort_routine') );
    $display_lists = 1;
    ...
    edit_class_sort_rule( $template, $rule_code, $input->param('description'), $input->param('sort_routine') );
    $display_lists = 1;
} else {
    $display_lists = 1;
}
