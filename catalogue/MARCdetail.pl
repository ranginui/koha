...
for ( my $tabloop = 0 ; $tabloop <= 10 ; $tabloop++ ) {
    ...
    for ( my $x_i = 0 ; $x_i <= $#fields ; $x_i++ ) {
        ...
        if ( $fields[$x_i]->tag() < 10 ) {
            ...
            for my $i ( 0 .. $#subf ) {
				if ( ($fields[$x_i]->tag() < 400) or ($fields[$x_i]->tag() > 499) ) {
					$subf[$i][0] = "@" unless $subf[$i][0];
				}
                next
                  if ( $tagslib->{ $fields[$x_i]->tag() }->{ $subf[$i][0] }->{tab} ne $tabloop );
                next
        ...
        if ( $#subfields_data >= 0 ) {
            ...
            if ( $fields[$x_i]->tag() eq $fields[ $x_i - 1 ]->tag() && ( C4::Context->preference('LabelMARCView') eq 'economical' ) ) {
                ...
                if ( C4::Context->preference('hide_marc') ) {
                    ...
                    $tag_data{tag} = $fields[$x_i]->tag() . ' ' . C4::Koha::display_marc_indicators( $fields[$x_i] ) . ' - ' . $tagslib->{ $fields[$x_i]->tag() }->{lib};
                }
            }
            my @tmp = @subfields_data;
            $tag_data{subfield} = \@tmp;
...
foreach my $field (@fields) {
    ...
    for my $i ( 0 .. $#subf ) {
        ...
        $this_row{ $subf[$i][0] } = GetAuthorisedValueDesc( $field->tag(), $subf[$i][0], $subf[$i][1], '', $tagslib ) || $subf[$i][1];
