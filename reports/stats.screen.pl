...
foreach my $payment (@payments) {
    ...
    if ( $payment->{'type'} ne 'writeoff' ) {
        ...
        @charges = getcharges( $payment->{'borrowernumber'}, $payment->{'timestamp'}, $payment->{'proccode'} );
        $totalcharges++;
        my $count = @charges;
...
if ( $op eq 'To Excel' ) {
    ...
    for my $row (@loop1) {
        my @array = (
            $row->{'branch'},      $row->{'datetime'}, $row->{'surname'}, $row->{'firstname'}, $row->{'description'},
            $row->{'accounttype'}, $row->{'amount'},   $row->{'type'},    $row->{'value'}
        );
    ...
    for my $row (@loop2) {
        ...
        my @array = (
            $row->{'creditbranch'},      $row->{'creditdate'},        $row->{'creditsurname'}, $row->{'creditfirstname'},
            $row->{'creditdescription'}, $row->{'creditaccounttype'}, $row->{'creditamount'}
        );
    ...
    for my $row (@loop3) {
        my @array = (
            $row->{'refundbranch'},      $row->{'refunddate'},        $row->{'refundsurname'}, $row->{'refundfirstname'},
            $row->{'refunddescription'}, $row->{'refundaccounttype'}, $row->{'refundamount'}
        );
