...
while ( $i < $count ) {
    ...
    my @rows2 = (
        $credits[$i]->{'branchcode'},  $credits[$i]->{'date'},        $credits[$i]->{'surname'}, $credits[$i]->{'firstname'},
        $credits[$i]->{'description'}, $credits[$i]->{'accounttype'}, "\$" . $credits[$i]->{'amount'}
    );
