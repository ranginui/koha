...
if ($do_it) {
    my $results = calculate( $line, $column, $podsp, $rodsp, $aodsp, $calc, \@filters );
    if ( $output eq "screen" ) {
...
sub calculate {
    ...
    my $strsth;
    $strsth .= "SELECT DISTINCTROW $linefield FROM (aqorders, aqbasket )
    ...
    my $strsth2;
    $strsth2 .= "SELECT distinctrow $colfield FROM (aqorders, aqbasket )
                 LEFT JOIN items ON (aqorders.biblionumber= items.biblionumber)
                 LEFT JOIN biblioitems ON (aqorders.biblionumber= biblioitems.biblionumber)
                 LEFT JOIN aqbudgets  ON (aqorders.budget_id = aqbudgets.budget_id )
