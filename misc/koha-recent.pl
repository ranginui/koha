...
<table width="100%">';
    ...
    my $sth = $dbh->prepare($serials_query);
    $sth->execute();
    while ( my $row = $sth->fetchrow_hashref() ) {
        print <<MAIN
<tr><td valign="top" align="left" border="0" class="newjournal">
<a href="$conf->{'kohaOpacUrl'}/opac-detail.pl?biblionumber=$row->{'biblionumber'}"><b class="newjournaltitle">$row->{title}</b></a> - <a href="$conf->{'kohaOpacUrl'}/opac-detail.pl?biblionumber=$row->{'biblionumber'}" class="newjournalenumchron">$row->{enumchron}</a>
</td></tr>
MAIN
          ;
    }
