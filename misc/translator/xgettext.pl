...
sub token_negligible_p( $ ) {
    ...
    return !$extract_all_p && (
        ...
        : $t == TmplTokenType::TEXT_PARAMETRIZED && join(
            '',
            map {
                my $t = $_->type;
                $t == TmplTokenType::DIRECTIVE ? '1' : $t == TmplTokenType::TAG ? '' : token_negligible_p($_) ? '' : '1'
              } @{ $x->children }
        ) eq ''
    );
}
...
sub string_list () {
    ...
    @t = sort {
        ...
        my @bb =
          sort { $a->pathname cmp $b->pathname || $a->line_number <=> $b->line_number } @{ $text{$b} };
    ...
    for my $t (string_list) {
        ...
        if ( $text{$t}->[0]->type == TmplTokenType::TEXT_PARAMETRIZED ) {
            ...
            for my $param ( $token->parameters_and_fields ) {
                ...
                my $type = $param->type;
                my $subtype =
                  ( $type == TmplTokenType::TAG && $param->string =~ /^<input\b/is ? $param->attributes->{'type'}->[1] : undef );
                ...
                if ( $type == TmplTokenType::DIRECTIVE ) {
                    ...
                    printf OUTPUT "#. %s: %s\n", $fmt, "$type" . ( defined $name ? " name=$name" : '' );
                } else {
                    my $name  = $param->attributes->{'name'};
                    my $value = $param->attributes->{'value'}
                      unless $subtype =~ /^(?:text)$/;
                    printf OUTPUT "#. %s: %s\n", $fmt, "type=$subtype" . ( defined $name ? " name=$name->[1]" : '' ) . ( defined $value ? " value=$value->[1]" : '' );
                }
            }
            ...
            if ( $token->string =~ /^<meta\b/is ) {
                ...
                my $tag  = uc($1);
                my $type = ( lc($tag) eq 'input' ? $token->attributes->{'type'} : undef );
                my $name = $token->attributes->{'name'};
                printf OUTPUT "#. %s\n", $tag . ( defined $type ? " type=$type->[1]" : '' ) . ( defined $name ? " name=$name->[1]" : '' );
            }
        ...
        printf OUTPUT "msgid %s\n", TmplTokenizer::quote_po TmplTokenizer::string_canon TmplTokenizer::charset_convert $t, $charset_in, $charset_out;
        printf OUTPUT "msgstr %s\n\n", ( defined $translation{$t} ? TmplTokenizer::quote_po( $translation{$t} ) : "\"\"" );
    }
}
