...
sub MapItemsToHoldRequests {
    ...
    foreach my $request (@$hold_requests) {
        ...
        if (exists $items_by_branch{$pickup_branch}
            and not($items_by_branch{$pickup_branch}->[0]->{holdallowed} == 1
                and $request->{borrowerbranch} ne $items_by_branch{$pickup_branch}->[0]->{homebranch} )
          ) {
            ...
            foreach my $branch (@pull_branches) {
                next
                  unless exists $items_by_branch{$branch}
                      and not($items_by_branch{$branch}->[0]->{holdallowed} == 1
                          and $request->{borrowerbranch} ne $items_by_branch{$branch}->[0]->{homebranch} );
...
sub CreatePicklistFromItemMap {
    ...
    foreach my $itemnumber ( sort keys %$item_map ) {
        ...
        $sth_load->execute(
            $biblionumber, $itemnumber, $barcode,        $surname,       $firstname,  $phone,        $borrowernumber, $cardnumber,
            $reservedate,  $title,      $itemcallnumber, $holdingbranch, $pickbranch, $reservenotes, $item_level
        );
    }
}
...
sub AddToHoldTargetMap {
    ...
    foreach my $itemnumber ( keys %$item_map ) {
        my $mapped_item = $item_map->{$itemnumber};
        $sth_insert->execute( $mapped_item->{borrowernumber}, $mapped_item->{biblionumber}, $itemnumber, $mapped_item->{holdingbranch}, $mapped_item->{item_level} );
    }
}
