...
my $readsth = $dbh->prepare(
    "SELECT id,biblio_auth_number,operation,server FROM zebraqueue WHERE done=0 
                           ORDER BY id DESC"
);    # NOTE - going in reverse order to catch deletes that
...
while ( ( $id, $biblionumber, $operation, $server ) = $readsth->fetchrow ) {
    ...
    eval {
        ...
        if ( $operation =~ /delete/i ) {    # NOTE depending on version, delete operation
            ...
            eval { my $hashed = XMLin($marcxml); };    ### is it a proper xml? broken xml may crash ZEBRA- slow but safe
