...
while ( my ($biblionumber) = $sth->fetchrow ) {
    ...
    print "\r$i";
    my $record;
    eval { $record = GetMarcBiblio($biblionumber); };
    if ($@) {
...
foreach my $key ( keys %result ) {
    foreach my $index ( keys %{ $result{$key} } ) {
        if ( length( $result{$key}->{$index} ) > 1000000 ) {
            print "very long index ("
              . length( $result{$key}->{$index} )
              . ")for $key / $index. update mySQL config file if you have an error just after this warning (max_paquet_size parameter)\n";
        }
        print "\r$i";
        $i++;
...
while ( my ($authid) = $sth->fetchrow ) {
    ...
    print "\r$i";
    my $record;
    eval { $record = GetAuthority($authid); };
    if ($@) {
