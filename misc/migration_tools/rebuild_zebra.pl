...
sub index_records {
    ...
    if ( $skip_export && $verbose_logging ) {
        ...
        if ($process_zebraqueue) {
            ...
            $num_records_exported = export_marc_records_from_list( $record_type, $entries, "$directory/upd_$record_type", $as_xml, $noxml );
            mark_zebraqueue_batch_done($entries);
        } else {
...
|;
    ...
    my $modulesdir;
    foreach (
        qw(/usr/local/lib/idzebra-2.0/modules/mod-grs-xml.so
...
|;
    ...
    my $tabdir;
    foreach (
        qw(/usr/local/share/idzebra/tab/explain.att
