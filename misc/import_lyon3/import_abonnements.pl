...
our (
    ...
    $add,        $limitimport, $limiterrors,     $matchingpoint,    $help,       $verbose,      $table,  $map,
    $valuemap,   $separator,   $postprocess,     $globalpreprocess, $preprocess, $valuemapfile, $delete
);
GetOptions(
...
if ( $file_input && length($file_input) > 0 ) {
  ...
  LINE: while ( defined( my $line = <$handle> ) ) {
        ...
        if ($subscriptionid) {
            ...
            if ($update) {
                ...
                if ($subscriptionid = NewSubscription(
                        0,
                        @$targetdata{
                            qw( branchcode  aqbooksellerid  cost
                              aqbudgetid    biblionumber startdate       periodicity
                              dow           numberlength weeklength      monthlength
                              add1          every1       whenmorethan1   setto1
                              lastvalue1    innerloop1   add2            every2
    ...
    my $PARAMS    = {
        ...
        'mappingvalues' => [ map { mapoutputkeys( [ $_, mapoutput( $valuesmapping->{$_} ) ], [ "field", "values" ] ) } keys %$valuesmapping ],
        'startdate'     => $startdate,
        'enddate'       => printtime(),
        'timespent'     => $timespent,
        'filename'      => $file_input,
