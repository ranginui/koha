...
BEGIN {
    ...
    use lib "$FindBin::Bin/../sudoc/sudoc/src/";
    use Dedoublonnage;
}
...
Parameters:
  ...
  yamlinput file  inputhinput format a yaml file with ids
...
my ( $dd, $conf, $yamlauth );
if ($yamlfileinput) {
    $conf = YAML::LoadFile(qq($FindBin::Bin/config_import.yaml));
    $dd = Dedoublonnage->new( $conf->{'ftp'}->{'localdir'}, $yamlfileinput );
}
            ...
            if ( $authorities && $marcFlavour ) {
                ...
                if ( $marcrecord->field('005')->data >= $record->field('005')->data ) {
                    if ($yamlfile) {
                        ...
                        my @subfields = map {
                            my $field = $_;
                            map { ( $_->[0] =~ /[a-z]/ ? $_->[1] : () ) } $field->subfields();
                        } $marcrecord->field("2..");
        ...
        if ($biblionumber) {
            ...
            if ($update) {
                ...
                if ($@) {
                    ...
                    printlog( { id => $originalid || $id || $biblionumber, op => "update", status => "ok" } ) if ($logfile);
                }
            } else {
                printlog( { id => $originalid || $id || $biblionumber, op => "insert", status => "warning : already in database" } ) if ($logfile);
            }
        } else {
            if ($insert) {
                ...
                printlog( { id => GetRecordId( $record, $tagid, $subfieldid ), op => "update", status => "warning : not in database;" . $record->field('001')->data() } )
                  if ($logfile);
            }
        }
        ( !$test_parameter && $biblionumber && $biblioitemnumber )
          and eval { ( $itemnumbers_ref, $errors_ref ) = AddItemBatchFromMarc( $record, $biblionumber, $biblioitemnumber, '' ); };
        if ($@) {
