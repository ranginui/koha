...
sub new {
    ...
    if ( $args{resumptionToken} ) {
        ( $metadata_prefix, $offset, $from, $until ) = split( ':', $args{resumptionToken} );
    } else {
...
sub new {
    ...
    $self->record( C4::OAI::Record->new( $repository, $marcxml, $timestamp, %args ) );
...
sub stylesheet {
    ...
    unless ($stylesheet) {
        my $xsl_file =
          ...
          : ( C4::Context->config('intranetdir') . "/koha-tmpl/intranet-tmpl/prog/en/xslt/" . C4::Context->preference('marcflavour') . "slim2OAIDC.xsl" );
        my $parser    = XML::LibXML->new();
