    ...
    if ( $server && $server =~ /biblioserver/ ) {    # this is the local bibliographic server
        ...
        if ($build_grouped_results) {
            foreach my $group ( @{ $results_hashref->{$server}->{"GROUPS"} } ) {
                ...
                my @group_results =
                  searchResults( $query_desc, $group->{'group_count'}, $results_per_page, 0, $scan, @{ $group->{"RECORDS"} }, C4::Context->preference('hidelostitems') );
        ...
        if ( C4::Context->preference('EnableOpacSearchHistory') ) {
            ...
            if ( defined $borrowernumber and $borrowernumber eq '' ) {

                # To a cookie (the user is not logged in)

                if ( exists($params->{'offset'}) and $params->{'offset'} eq '' ) {
