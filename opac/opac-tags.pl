...
sub ajax_auth_cgi ($) {    # returns CGI object
    ...
    if ( $auth_status ne "ok" ) {
        output_with_http_headers $input, undef, "window.alert('Your CGI session cookie ($sessid) is not current.  " . "Please refresh the page and try again.');\n", 'js';
        exit 0;
    }
...
if ( scalar @newtags_keys ) {
    ...
    foreach my $biblionumber (@newtags_keys) {
        ...
        foreach (@values) {
            ...
            my $result = ($openadds) ? add_tag( $biblionumber, $clean_tag, $loggedinuser, $loggedinuser ) :    # pre-approved
...
if ($add_op) {
    ...
    foreach ( reverse @$results ) {
        ...
        if ($strata < $maxstrata
            and (  $cloud{$strata} > $chunk
                or $count - $total <= $maxstrata - $strata )
          ) {
            $strata++;
        }
        $_->{cloudweight} = $strata;
