...
my %requirefields = (
    subscription => {
        ...
        'distributedto'  => 'text NULL',
        'firstacquidate' => 'date default NULL',
        'irregularity'   => 'TEXT NULL default \'\'',
        'numberpattern'  => 'TINYINT(3) NULL default 0',
        'callnumber'     => 'text NULL',
        'hemisphere'     => 'TINYINT(3) NULL default 0',
        'issuesatonce'   => 'TINYINT(3) NOT NULL default 1',
        'branchcode'     => 'varchar(10) NOT NULL default \'\'',
        'manualhistory'  => 'TINYINT(1) NOT NULL default 0',
        'internalnotes'  => 'LONGTEXT NULL default \'\''
    },
    ...
    foreach my $row (@$fieldrow) {
        ...
        unless ( $type eq $def->{type}
            ...
            if ( $key ne 'PRIMARY KEY' ) {
                ...
                $query = "alter table $table $action `$field` $type $null $key $extra " . GetDefaultClause($default) . " $after";
            } else {
                ...
                # strange...
                $query = "alter table $table drop primary key, $action `$field` $type $null $key $extra " . GetDefaultClause($default) . " $after";
                $query = "alter table $table $action `$field` $type $null $key $extra " . GetDefaultClause($default) . " $after";
            }
