...
my @categories = @{ $dbh->selectall_arrayref( 'SELECT description, categorycode FROM categories WHERE overduenoticerequired > 0', { Slice => {} } ) };
...
if ( $op eq 'save' ) {
    ...
    foreach my $bor ( keys %temphash ) {
        ...
        if ( $temphash{$bor}->{delay1} =~ /[^0-9]/ and $temphash{$bor}->{delay1} ne "" ) {
            ...
            $err = 1;
        } elsif ( $temphash{$bor}->{delay3} and ( $temphash{$bor}->{delay3} <= $temphash{$bor}->{delay2} or $temphash{$bor}->{delay3} <= $temphash{$bor}->{delay1} )
