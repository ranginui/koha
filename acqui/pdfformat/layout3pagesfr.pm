...
sub printorders {
    ...
    for my $basket (@$baskets) {
        ...
        my $abaskets;
        my $arrbasket;
        my @keys = ( 'Document', 'Qte', 'Prix', 'Remise', 'Prix net', 'TVA', 'Total TTC' );
        for my $bkey (@keys) {
            push( @$arrbasket, $bkey );
        }
...
sub printbaskets {
    ...
    # header of the table
    my @keys = ( 'Lot', 'Panier', 'Prix', 'Remise', 'Prix net', 'Taux TVA', 'Total HT', 'TVA', 'Total TTC' );
    for my $bkey (@keys) {
        push( @$arrbasket, $bkey );
    }
    my $grandtotal = 0;
    ...
    for my $basket (@$hbaskets) {
        ...
        $grandgst   += $gst;
        push( @$arrbasket,
            $basket->{contractname},                             $basket->{basketname} . '(N°' . $basket->{basketno} . ')', $num->format_price($totallist),
            $num->format_price( $bookseller->{discount} ) . '%', $num->format_price($total),                                 $num->format_price( $GSTrate * 100 ) . '%',
            $num->format_price( $total - $gst ),                 $num->format_price($gst),                                   $num->format_price($total) );
        push( @$abaskets, $arrbasket );
    }

    # now, push total
    undef $arrbasket;
    push @$arrbasket, '', '', '', 'Total', $num->format_price($grandtotal), '', $num->format_price( $grandtotal - $grandgst ), $num->format_price($grandgst),
      $num->format_price($grandtotal);
    push @$abaskets, $arrbasket;
