...
for (my $i = 0 ; $i < $countlines ; $i++) {
    ...
    $line{gst} = $parcelitems[$i]->{'gst'};
    $gst += $parcelitems[$i]->{'gst'};
...
$template->param(
    ...
    grandtot              => sprintf($cfstr, $tototal+$gst), # FIXME assumes budgeted cost is gst exclusive (currently correct), will need to be fixed once a pref exists for gst incl/excl gst
