    ...
    for my $biblio (@$biblios) {
        if ( $cgiparams->{ 'order-' . $biblio->{'import_record_id'} } ) {
            ...
            my %orderinfo = (
                ...
                "branchcode", $branch, "booksellerinvoicenumber", $invoice, "budget_id", $budget_id, "uncertainprice", 1,
            );
            ...
            if ( C4::Context->preference('AcqCreateItem') eq 'ordering' ) {
                ...
                my @itemids =
                  $input->param('itemid');  # hint : in iso2709, the itemid contains the import_record_id, not an item id. It is used to get the right item, as we have X biblios.
                ...
                for my $itemid (@itemids) {
                    my $realitemid
                      ;    #javascript generated random itemids, in the form itemid-randomnumber, $realitemid is the itemid, while $itemid is the itemide parsed from the html
                    if ( $itemid =~ m/(\d+)-.*/ ) {
                        my @splits = split( /-/, $itemid );
                        $realitemid = $splits[0];
                    }
                    if (   ( $realitemid && $cgiparams->{ 'order-' . $realitemid } && $realitemid eq $biblio->{import_record_id} )
                ...
                foreach my $item (@items) {
                    my $xml = TransformHtmlToXml( $item->{'tags'}, $item->{'subfields'}, $item->{'field_values'}, $item->{'ind_tag'}, $item->{'indicator'} );
